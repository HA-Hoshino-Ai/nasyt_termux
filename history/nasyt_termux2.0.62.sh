...
pkg_install() {
    ...
    if command -v $pkg_install_app >/dev/null 2>&1; then
        ...
        pkg install -y $pkg_install_app 2>&1 >/dev/null
        if [ $? -ne 0 ]; then
            echo -e "$green 安装成功 $color"
        else
                ...
                5)
                    ...
                    do
                        ...
                        case $acg_menu_xz in
                            ...
                            3)
                                clear
                                save_place=$($habit --title "修改保存路径(末尾自动带/)" \
                                --inputbox "当前保存路径：$save_place" 0 0 \
                                2>&1 1>/dev/tty)
                                clear
                                echo -e "路径已更改为:$blue$save_place/$color!"
                                echo -e "点击$blue回车$color返回🤓"
                                read
                                ;;
                            4)
                                break
...
main_home(){
    ...
    --menu "当前版本:$version 更新时间：$time_date\n本工具箱由NAS油条xHA制作\nQQ群:610699712\n请使用方向键+回车键进行操作\n请选择你要启动的项目：" 0 0 10 \
    1 "系统工具" \
    2 "基础菜单" \
    3 "实验脚本" \
    4 "其他工具" \
