...
pkg_install() {
    ...
    if command -v $pkg_install_app >/dev/null 2>&1; then
        ...
        apt-get -qq install $pkg_install_app
        if [ $? -ne 0 ]; then
            echo -e "$green $pkg_install_app 安装失败 $color"
        else
            echo -e "$red $pkg_install_app 安装成功 $color"
        fi
    fi
}
...
habit_choose(){
    local config_file="$HOME/.nasyt/naster_config.conf"
    if [ -f "$config_file" ]; then
        habit=$(cat "$config_file")
        if [ "$habit" = "dialog" ] || [ "$habit" = "whiptail" ]; then
            echo -e "已加载上次设置：$green$habit$color"
            return 0
        fi
    fi
    echo "$habit" > "$config_file"
    while true
    do
        echo "请选择触控方式🤓："
        echo "1.dialog点击式"
...
start_dialog_proot(){
    ...
    if [ ! -d "$INSTALL_DIR" ]; then
        dialog --msgbox "未找到 proot-distro 安装目录: $INSTALL_DIR" 10 50
        exit 1
    fi
    containers=($(ls -1 "$INSTALL_DIR"))
    if [ ${#containers[@]} -eq 0 ]; then
        dialog --msgbox "没有找到任何已安装的容器。" 10 40
        exit 0
    fi
    menu_options=()
...
about_naster(){
    ...
    echo -e "naster贡献名单："
    echo -e "$blue NAS油条 $color        为naster提供了$gblue 基础功能 $color以及naster的$blue 其他功能 $color!"
    echo -e "$blue ElectricityDream $color 为naster提供了$gblue 建议 $color以及naste的r$blue 其他功能 $color!"
...
mit_gancm(){
#Mit_gancm脚本
    mit_gancm_xz=$($habit --title "Minecraft服务器部署" \
    --menu "该功能由Mit_gancm提供，未经授权，严禁搬运/盗用！" 0 0 10 \
    1 
    2>&1 1>/dev/tty)
}
                ...
                do
                    ...
                    case $acg_menu_xz in
                        ...
                        3)
                            ...
                            if [ -z "$save_place" ]; then
                                save_place="$HOME/nasyt" # 默认路径
                            fi
                            $habit --title "修改保存路径(末尾自动带/)" \
                            --inputbox "当前保存路径: $save_place" 0 0 \
                            2>&1 1>/dev/tty
                            save_place=$(cat /dev/tty)
                            save_place=$(echo "$save_place" | xargs)
                            if [ -z "$save_place" ]; then
                                save_place="$HOME/.nasyt/acg"
                            fi
                            if [[ ! "$save_place" =~ /$ ]]; then
                            ...
                            sed -i "s/^save_place=.*/save_place=$save_place/" "$config_file"
                            clear
                            echo -e "$blue 路径已更改为: $save_place $color"
                            echo -e "$blue 点击 $blue 回车 $color 返回 $color"
                            read
                            break
                            ;;
                        4)
