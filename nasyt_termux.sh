...
convert_single_ncm(){
    ...
    $habit --title "转换信息" \
        --msgbox "即将转换:\n\n文件名：$ncm_filename\n路径：$ncm_file\n输出目录：$output_dir" 12 60
    local mp3_file="${ncm_file%.ncm}.mp3"
    echo "正在转换: $ncm_filename"
    if ncmdump "$ncm_file" >/dev/null 2>&1; then
        if [ -f "$mp3_file" ]; then
            local mp3_size=$(du -h "$mp3_file" 2>/dev/null | cut -f1)
            $habit --title "转换成功" \
            --msgbox "转换成功！\n原文件：$ncm_filename\n新文件：$(basename "$mp3_file")\n文件大小：$mp3_size" 12 60
        else
            $habit --title "转换失败" --msgbox "转换失败，未生成MP3文件" 0 0
        fi
    else
        $habit --title "转换失败" --msgbox "转换过程出错" 0 0
    fi
}
